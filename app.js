...
app.use(BASE_URL + "/user", require("./routes/users"));
// app.use(BASE_URL + "/our-teams", require("./routes/our_teams"));
app.use(BASE_URL + "/packages", require("./routes/packages"));
app.use(BASE_URL + "/mypackage", require("./routes/my_package"));
...
app.use(function (err, req, res, next) {
  if (err.status === 400) {
    return res
      .status(response_codes.CODE_BAD_REQUEST)
      .send(
...
app.use(function (req, res, next) {
  res
    .status(response_codes.CODE_NOT_FOUND)
    .send(
      common_utils.response_generator(
...
app.use(function (err, req, res, next) {
  ...
  console.error(err);
  res
    .status(response_codes.CODE_INTERNAL_SERVER_ERROR)
    .send(
      common_utils.response_generator(
